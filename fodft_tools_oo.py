...
class fodft:
    ...
    def __init__(self, dimer, fformat="xyz"):
        ...
        #self.fo_embedding = False
        #self.fo_orbitals = None
        self.fo_type = "hole"
        self.frontiers = [1, 1]
        self.fo_range = [2, 2]
...
class fo_aims(fodft):
    ...
    def update_calculators(self):
        ...
        self.frag1.calc.set(default_initial_moment=abs(self.charges['frag1']), 
                            fo_dft="fragment",
                            fo_embedding=self.embedding,
                            charge=self.charges['frag1'])
        self.frag2.calc.set(default_initial_moment=abs(self.charges['frag2']), 
                            fo_dft="fragment", 
                            fo_embedding=self.embedding,
                            charge=self.charges['frag2'])

        self.dimer.calc.set(default_initial_moment=abs(self.charges['fo']), 
                         fo_dft="final", 
